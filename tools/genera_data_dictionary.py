...
GRUPPI = [
    ...
    ("Categorizzazione delle attività e dei servizi (Det. ACN 155238/2026)", ['modello_categorizzazione',
      'macro_area','categoria_rilevanza','macro_area_modello']),
...
out = []
out.append("# Data dictionary – schemi `nis2` e `nis2_meta`\n")
...
# controllo di completezza: ogni tabella dello schema deve comparire in un gruppo
tutte = {r[0] for r in q("SELECT tablename FROM pg_tables WHERE schemaname='nis2' AND tablename NOT LIKE '%\\_storico'")}
elencate = {t for _, tabs in GRUPPI for t in tabs}
altre = sorted(tutte - elencate)
if altre:
    GRUPPI.append(("Altre tabelle", altre))
GRUPPI = [(tit, [("nis2", t) for t in tabs]) for tit, tabs in GRUPPI]
GRUPPI.append(("Metadati di versione (schema nis2_meta)", [("nis2_meta", "versione_schema"), ("nis2_meta", "registro_deploy")]))

for titolo, tabs in GRUPPI:
    out.append(f"\n## {titolo}\n")
    for sch, t in tabs:
        desc = q(f"SELECT obj_description('{sch}.{t}'::regclass,'pg_class')")[0][0]
        nrows = q(f"SELECT count(*) FROM {sch}.{t}")[0][0]
        ...
        cols = q(f"""
          SELECT a.attname, format_type(a.atttypid,a.atttypmod), a.attnotnull::text,
           ...
           WHERE a.attrelid='{sch}.{t}'::regclass AND a.attnum>0 AND NOT a.attisdropped ORDER BY a.attnum""")
        ...
        cons = q(f"""SELECT conname, pg_get_constraintdef(oid) FROM pg_constraint
                     WHERE conrelid='{sch}.{t}'::regclass AND contype IN ('c','x','u','f') ORDER BY contype, conname""")
        if cons:
            out.append("\n**Vincoli**\n")
            for n, dfn in cons:
                out.append(f"- `{n}`: `{md(dfn)}`")
        idx = q(f"""SELECT indexname, indexdef FROM pg_indexes WHERE schemaname='{sch}' AND tablename='{t}'
                    AND indexname NOT IN (SELECT conname FROM pg_constraint WHERE conrelid='{sch}.{t}'::regclass)
                    ORDER BY 1""")
        if idx:
            out.append("\n**Indici secondari**\n")
            for n, dfn in idx:
                out.append(f"- `{n}`: `{md(dfn.split(' USING ',1)[-1])}`")
        trg = q(f"""SELECT tgname FROM pg_trigger WHERE tgrelid='{sch}.{t}'::regclass AND NOT tgisinternal ORDER BY 1""")
        if trg:
